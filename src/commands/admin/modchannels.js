...
module.exports = class ModChannelsCommand extends Command {
  constructor(client) {
    super(client, {
      name: 'modchannels',
      aliases: ['setmods'],
      usage: 'modchannels <channel mentions/IDs>',
      description: oneLine`
        ...
        and bot will only respond to members with permission to use those commands.
        Provide no channels to clear the current \`mod channels\`.
      `,
      type: client.types.ADMIN,
      userPermissions: ['MANAGE_GUILD'],
      examples: ['modchannels #general #memes #off-topic']
    });
  }
  async run(message, args) {

    const { trimArray  } = message.client.utils;
    const modChannelIds = util.mod_channels(message.guild.id);
    let oldModChannels = [];
    if (modChannelIds) {
