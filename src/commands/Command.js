...
class Command {
	...
	sendCooldownMessage(message, cooldown) {
		...
		const seconds =
			this.cooldown - Math.floor((Date.now() - cooldown.start) / 1000);
		const embed = new MessageEmbed()
			.setAuthor(`${author.tag}`, author.displayAvatarURL({ dynamic: true }))
			.setTitle(`Cooldown Active: \`${this.name}\``)
			.setDescription(
				`This command is on cooldown for another \`${seconds}\` second${
					seconds != 1 ? 's' : ''
				}.`
			)
			.setTimestamp()
	...
	async getUser(message, args) {
		let User = await userSet.findOne({
			user_id: message,
			guild_id: args
		});
		return User;
